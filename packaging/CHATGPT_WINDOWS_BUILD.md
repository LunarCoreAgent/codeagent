...
版本号从 `pyproject.toml` 的 `version` 读取（当前 `0.75.1`）。
...
```bat
"%ProgramFiles(x86)%\Inno Setup 6\ISCC.exe" /DMyAppVersion=0.75.1 /Fcodeagent-desktop-windows-amd64-setup packaging\windows-setup.iss
```
