# CodeCoreAgent Windows 封装配置说明（v0.51.0）
...
版本号来源：**`pyproject.toml` 的 `version = "0.51.0"`**。以下位置须一致：

| 位置 | 当前值 |
|------|--------|
| `pyproject.toml` | `0.51.0` |
| `src/codeagent/__init__.py` | `__version__ = "0.51.0"` |
| `src/codeagent/releases.py` | `Release("0.51.0", ...)` |
| `CHANGELOG.md` | `## 0.51.0（2026-09-10）` |
| `packaging/codeagent-desktop.spec` | `CFBundleShortVersionString: "0.51.0"`（macOS） |
| `packaging/windows-setup.iss` | `#define MyAppVersion "0.51.0"`（`build_desktop.py` 会用 `/DMyAppVersion=` 覆盖） |
...
```bat
"%ProgramFiles(x86)%\Inno Setup 6\ISCC.exe" /DMyAppVersion=0.51.0 /Fcodeagent-desktop-windows-amd64-setup packaging\windows-setup.iss
```
